...
use backend\models\search\shop\ProductImageSearch;
...
//                    'delete' => ['post'],
    ...
    public function actionAjaxUpload($product_id)
    {
        ...
        if (Yii::$app->request->isAjax && $model->validate()) {
            ...
            foreach (UploadedFile::getInstances($model, 'name') as $file) {
                ...
                $new_file->product_id = $product_id;
    ...
    /**
     * Delete an existing ProductImage model.
     * For ajax request will return json object
    ...
    public function actionDelete($id, $product_id = null)
    {
        ...
        if ($request->isAjax) {
            ...
            if ($model) {
                $dir = Yii::getAlias('@frontendWeb/img/products/');
                if (file_exists($dir . $product_id . '/' . $model->name)) {
                    unlink($dir . $product_id . '/' . $model->name);
                } else {
                    unlink($dir . $model->name);
                }
                $model->delete();
            }
            return ['forceClose' => true, 'forceReload' => '#view-pjax'];
        } else {
    ...
    /**
     * Delete multiple existing ProductImage model.
     * For ajax request will return json object
     * and for non-ajax request if deletion is successful, the browser will be redirected to the 'index' page.
     * @param integer $id
     * @return mixed
